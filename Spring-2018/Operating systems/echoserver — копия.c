...
//    fflush( stdout );
    ...
    /*Client's socket is closed, the thread is exited and the number of clients is decremented whenever the server notices (usually by trying to read or write) that the client disconnects */
        ...
        if(count == 1){
            firstSocket = ssock; //first socket number
            numberOfNames = 0;
            resCount = 0;
...
//                printf( "The client's answer: %s\n", splitted[1]);
    ...
    if(res < numberOfClients){
        ...
        pthread_mutex_lock( &mutex );
            for(int i = firstSocket; i < numberOfClients+firstSocket-1; i++){
                for(int j = firstSocket; j < numberOfClients+firstSocket-1; j++){
        ...
        strcpy(R, "RESULT");
        /*Iterate through the *Lname[] and scores[] arrays to write the results*/
