...
void *serveClient(void *arg){
   ...
   // struct client *cl;
    ...
    /*Client's socket is closed, the thread is exited and the number of clients is decremented whenever the server notices (usually by trying to read or write) that the client disconnects */
        ...
        if(Lnum!=0){
            isFull = 1; //once the the number of clients reaches group size nobody can connect
        }
        count = exitGroup(cc, count);
...
//                printf("I got5\n");
                ...
                if(isF == 0 && strcmp(answ, splitted[1]) == 0){
                    ...
                    isF = isFirst;
                    pthread_mutex_unlock( &mutex );
                }else if(isF == 1 && strcmp(answ, splitted[1]) == 0){
                    isF = 0;
                }
    ...
    char numbCl[1000];
    pthread_mutex_lock( &mutex );
    for(int i = firstSocket; i < numberOfClients+firstSocket; i++){
        for(int j = firstSocket; j < numberOfClients+firstSocket; j++){
    ...
    for(int j = firstSocket; j < numberOfClients+firstSocket; j++){
        ...
        sprintf(numbCl, "|%s", Lname[j]);
        strcat(R, numbCl);
        sprintf(numbCl, "|%i", scores[j]);
        strcat(R, numbCl);
    ...
    close(ssock);
    pthread_mutex_lock( &mutex );
    numberOfClients = 0; //decrement the number of clients
    pthread_mutex_unlock( &mutex );
    pthread_exit( NULL );
}
...
//            break;
    ...
    while ( fgets( buf, BUFSIZE-1, fin ) != NULL && i < 100 )
    {
